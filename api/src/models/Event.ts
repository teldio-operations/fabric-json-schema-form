...
/**
 * Retrieve events one-by-one in long polling style. If an event is not available yet, the server keeps the connection alive until an event is emitted. If Last-Event-ID is set, retrieves the oldest event after the specified ID.
 * @export
 * @interface Event
 */
